export default function PlayersEditingWindow({ showManageBox, setShowManageBox, players, handleRename, bool, setBool }) {
	return (
		<>
			<button
			...
			{showManageBox && (
				<div className="flex absolute w-[99%] h-[67%] gap-4 p-4 pb-6 pt-3 mt-3 overflow-x-auto scroll-smooth ps-scroll">
					{players.map((player) => (
						<div
							key={player.id}
							className="flex flex-col justify-center items-center gap-2 rounded-xl w-[23.2%] h-[100%] transition-all border-transparent border text-[20px] font-thin bg-[#3b3b3b83] hover:border-blue-500 backdrop-blur-[5px] flex-shrink-0"
							onClick={() => handleRename(player.id)}
						>
							<svg
								viewBox="0 0 24 24"
								fill="none"
								className="group flex justify-center items-center relative w-full h-[55%] cursor-pointer rounded-full"
